...
def run_script(data_source, lv, fl_round, device, condition):
    subprocess.run([
...
def main(fl_end_round, lv, condition):
    fl_rounds = range(1, fl_end_round)
    
    # Initialization
    if not os.path.exists(RESULT_PATH + f'Condition{condition}/lv{lv}/round0/shared_avg_layers.pth'):
        os.makedirs(RESULT_PATH + f'Condition{condition}/lv{lv}/round0/', exist_ok=True)
        generate_null_models(lv, condition)
        model_initialization(lv, 0, condition)

    for fl_round in fl_rounds:
        round_rst_path = RESULT_PATH + f'Condition{condition}/lv{lv}/round{fl_round}/'
        os.makedirs(round_rst_path, exist_ok=True)
        round_rst = round_rst_path + f'shared_avg_layers.pth'
        
        if os.path.exists(round_rst):
            print(f'Condition{condition}, Lv.{lv}: Round {fl_round} has already been completed.')
            continue
        else:
            # Parallelism start
            processes = [
                Process(target=run_script, args=(
                    dat_src, lv, fl_round, ((lv - 1) + DATA_SOURCE.index(dat_src)) % 4, condition
            ...
            print(f'Condition{condition}, Round {fl_round}: All the local set was trained.')

            model_initialization(lv, fl_round, condition)
            print(f'Condition{condition}, Lv.{lv}: Round {fl_round+1} has been initialized.')
...
if __name__ == "__main__":
    ...
    main(args.fl_end_round, args.lv, args.condition)