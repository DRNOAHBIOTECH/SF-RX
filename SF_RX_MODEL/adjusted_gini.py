...
def main(lv, n_inference, device): 
    ...
    temp_df = pd.DataFrame(columns = ['val_fold', 'sev', 'desc'])
    for val_fold in range(5):        
        ...
        for inference in range(n_inference):
            ...
            print(inference)
            with torch.no_grad():
                for a, b, _, _, _ in val_loader:
                    A = a.to(f'cuda:{device}', non_blocking=True)
                    B = b.to(f'cuda:{device}', non_blocking=True)
                    results_gpu = model(A, B)
                    temp_results_sev.append(results_gpu[0])
                    temp_results_desc.append(results_gpu[1])
                    temp_results_dir.append(results_gpu[2])
            results_sev.append(torch.cat(temp_results_sev, dim = 0))
            results_desc.append(torch.cat(temp_results_desc, dim = 0))
            results_dir.append(torch.cat(temp_results_dir, dim = 0))

        with open(RESULT_PATH+f"uncertainty/Adjusted/lv{lv}_val{val_fold}_sev_inf{n_inference}_prob.pkl", "wb") as f:
            pickle.dump(results_sev, f)
        with open(RESULT_PATH+f"uncertainty/Adjusted/lv{lv}_val{val_fold}_desc_inf{n_inference}_prob.pkl", "wb") as f:
            pickle.dump(results_desc, f)
        with open(RESULT_PATH+f"uncertainty/Adjusted/lv{lv}_val{val_fold}_dir_inf{n_inference}_prob.pkl", "wb") as f:
            pickle.dump(results_dir, f)
        ...
        dir_res = []
        for infs in range(n_inference):
            new_tensor = torch.cat([results_dir[infs], 1 - results_dir[infs]], dim=1)
            dir_res.append(torch.argmax(new_tensor, dim = 1))

        sev_ginis = load_preds(sev_res, 'sev')
        desc_ginis = load_preds(desc_res, 'desc')
        dir_ginis = load_preds(dir_res, 'dir')

        plt.hist(sev_ginis)
        plt.title(f'[Level {lv}, Validation {val_fold}, DDI Severity]: Historgram of Gini Impurity')
        plt.xlabel('Gini Impurity')
        plt.ylabel('Frequency')
        plt.savefig(RESULT_PATH+f"uncertainty/Adjusted/lv{lv}_Gini_Hist_Severity_{val_fold}_inf{n_inference}.png")
        plt.clf()
        ...
        plt.xlabel('Gini Impurity')
        plt.ylabel('Frequency')
        plt.savefig(RESULT_PATH+f"uncertainty/Adjusted/lv{lv}_Gini_Hist_Description_{val_fold}_inf{n_inference}.png") 
        plt.clf()

        plt.hist(dir_ginis)
        plt.title(f'[Level {lv}, Validation {val_fold}, DDI Direction]: Historgram of Gini Impurity')
        plt.xlabel('Gini Impurity')
        plt.ylabel('Frequency')
        plt.savefig(RESULT_PATH+f"uncertainty/Adjusted/lv{lv}_Gini_Hist_Direction_{val_fold}_inf{n_inference}.png") 
        plt.clf()
        
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SF-RX adjusted model for DDI Prediction")
